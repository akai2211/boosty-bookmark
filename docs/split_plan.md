...
### [ ] Этап 1b: Релизная сборка (`build.cjs`) и флаг `DEV`
...
### [ ] Этап 2: Финализация `src/locales.js` / `src/webdav-sync.js` и удаление дубликатов
...
### [ ] Этап 5: Выделение `src/sync.js` (Синхронизация и API) — 🔴 ВЫСОКИЙ риск, с разрывом цикла
...
### [ ] Этап 8: Выделение `src/navigation.js` (Навигация и скроллинг) — независимый, отложен
